<?php
    ...
    if (isset($_POST["mode"]) && $_POST["mode"] === "delete") {
        ...
        $sql = "delete from image where num = $num";
        mysqli_query($con, $sql);
        // 이미지 게시판 댓글 삭제
        $sql = "delete from image_ripple where parent = $num";
        mysqli_query($con, $sql);
        mysqli_close($con);
        echo "<script>
            location.href = 'image_list.php?page=$page';
        </script>";
        ...
        $sql = "insert into image(id, nick, subject, content, regist_day, hit, file_name, file_type, file_copied) ";
        ...
        if(($file_delete !== "yes") && empty($_FILES["upfile"]["name"])){
            $sql = "update image set subject='$subject', content='$content' where num=$num ";
            mysqli_query($con, $sql);
            ...
            $upfile_type = "";
            $copied_file_name = "";
            $sql = "update image set subject='$subject', content='$content',  file_name='$upfile_name', file_type='$upfile_type', file_copied= '$copied_file_name'";
            $sql .= " where num=$num";
            mysqli_query($con, $sql); 
        }else{
            if ($copied_name) {
        ...
        if (!$rowcount) {
            ...
            echo "
            <script>
              location.href='./image_view.php?num=$parent&page=$page&hit=$hit';
            </script>
            ";
        }//end of if rowcount
        ...
        $sql = "DELETE FROM `image_ripple` WHERE num=$q_num";
        $result = mysqli_query($con, $sql);
        if (!$result) {
            die('Error: ' . mysqli_error($con));
        }
        mysqli_close($con);
        echo "<script>
            location.href='./image_view.php?num=$parent&page=$page&hit=$hit';</script>";
    }
?>